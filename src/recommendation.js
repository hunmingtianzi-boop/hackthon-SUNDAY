const productData = require("./data/jd_skincare_products.json");

const DEFAULT_TOP_N = 6;
const DEFAULT_CATEGORY_LIMIT = 2;
const FALLBACK_WARNING = "当前预算或标签下匹配商品较少，已为你补充高评分的温和日常护理产品。";
...
function extractDemands(analysisResult = {}) {
  const issueList = toArray(analysisResult.issues);
  ...
  return {
    skinType: normalizeSkinType(analysisResult.skinType),
    concerns: [...concernWeights.keys()],
    concernWeights,
    issueTypes: uniq(issueTypes),
    isSensitive:
      normalizeSkinType(analysisResult.skinType) === "敏感性" ||
      concernWeights.has("泛红敏感") ||
      concernWeights.has("屏障受损"),
  };
}
...
function calculateCategoryScore(product, concerns) {
  ...
  const preferredCategories = uniq(
    concerns.flatMap((concern) => CATEGORY_PREFERENCES[concern] || [])
  );
...
function calculateRatingScore(product) {
  return clamp(product.rating / 5, 0, 1);
}
...
function calculateRiskPenalty(product, demands) {
  let penalty = 0;
  if (demands.isSensitive) {
    penalty += Math.min(product.contraindications.length * 2, 6);
    const riskText = `${product.name} ${product.description} ${product.contraindications.join(" ")}`;
    if (/酸|水杨酸|AHA|BHA|视黄醇|A醇|刷酸|剥脱/.test(riskText)) {
      penalty += 5;
    }
  }
  return penalty;
...
function scoreProduct(product, demands, options = {}) {
  ...
  const riskPenalty = calculateRiskPenalty(product, demands);
...
function generateWarnings(product, demands, isFallback) {
  const warnings = [...product.contraindications];
  if (demands.isSensitive && !warnings.includes(SENSITIVE_WARNING)) {
    warnings.push(SENSITIVE_WARNING);
  }
  if (isFallback && !warnings.includes(FALLBACK_WARNING)) {
    warnings.unshift(FALLBACK_WARNING);
  }
  return uniq(warnings);
}

function toRecommendation(product, scoreInfo, demands, isFallback = false) {
  return {
    product,
    ...
    warnings: generateWarnings(product, demands, isFallback),
...
function buildFallbackRecommendations(products, demands, options, reason) {
  const preferredConcerns = demands.isSensitive
    ? ["屏障受损", "泛红敏感", "日常护理"]
    : ["日常护理", ...demands.concerns];

  const fallbackProducts = products
    .map((product) => {
      const concernBoost = product.concerns.some((concern) => preferredConcerns.includes(concern)) ? 18 : 0;
      const skinBoost =
        product.skinTypes.includes(demands.skinType) || product.skinTypes.includes("多种肤质") ? 10 : 0;
      const budgetPenalty =
...
/**
 * 根据结构化肌肤分析结果，从本地商品库中返回可解释的护肤产品推荐。
 * @param {object} input 推荐输入，包含 analysisResult、budget、topN、categoryLimit。
 * @returns {Array<object>} ProductRecommendation 列表。
 */
function matchProducts(input = {}) {
  const analysisResult = input.analysisResult || {};
  if (analysisResult.isValidFace === false) return [];

  const topN = Number.isFinite(Number(input.topN)) ? Math.max(1, Number(input.topN)) : DEFAULT_TOP_N;
  const categoryLimit = Number.isFinite(Number(input.categoryLimit))
    ? Math.max(1, Number(input.categoryLimit))
    : DEFAULT_CATEGORY_LIMIT;
  const budget = Number.isFinite(Number(input.budget)) && Number(input.budget) > 0 ? Number(input.budget) : undefined;
  const options = { topN, categoryLimit, budget };
  const products = productData.map(normalizeProduct);
  ...
  if (budgetProducts.length === 0) {
    return buildFallbackRecommendations(products, demands, options, "budget");
  }

  const scored = budgetProducts
    .map((product) => {
      const scoreInfo = scoreProduct(product, demands, options);
      return toRecommendation(product, scoreInfo, demands);
    })
  ...
  if (!hasConcernMatch) {
    return buildFallbackRecommendations(budgetProducts, demands, options, "no-match");
  }
